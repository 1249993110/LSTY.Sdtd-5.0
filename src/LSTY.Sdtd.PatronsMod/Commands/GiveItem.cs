...
namespace LSTY.Sdtd.PatronsMod.Commands
{
    public class GiveItem : ConsoleCmdBase
    {
        ...
        {
            return "Usage:\n" +
                "  1. GiveItem <SteamId/EntityId> <ItemName> <Count> <Quality> <Durability>\n" +
                "  2. GiveItem <SteamId/EntityId> <ItemName> <Count> <Quality>\n" +
                "  3. GiveItem <SteamId/EntityId> <ItemName> <Count>\n" +
                "  4. GiveItem <SteamId/EntityId> <ItemName>\n" +
                "  5. GiveItem all <ItemName> <Count> <Quality> <Durability>\n " +
                "  6. GiveItem all <ItemName> <Count> <Quality>\n " +
                "  7. GiveItem all <ItemName> <Count>\n " +
                "  8. GiveItem all <ItemName>\n " +
        ...
        public override void Execute(List<string> args, CommandSenderInfo senderInfo)
        {
            try
            {
                ...
                if (argCount < 2 && args.Count > 5)
                {
                    Log(string.Format("Wrong number of arguments, expected 2 to 5, found {0}", args.Count));
                    return;
                }
                    ...
                    if (itemValue != null)
                    {
                        ...
                        if (arg0.ToLower() == "all")
                        {
                            foreach (var cInfo in ConnectionManager.Instance.Clients.List)
                            {
                                ...
                                {
                                    Log(string.Format("Player with steamd Id {0} has not spawned. Unable to give item", cInfo.playerId));
                                }
                            }
                        }
                        else
                        {
                            ClientInfo cInfo = ParseParamId(arg0);
                                ...
                                && player.IsSpawned())
                            {
                                ...
                                Log(string.Format("Gave {0} to {1}", itemValue.ItemClass.GetLocalizedItemName() ?? itemValue.ItemClass.Name, cInfo.playerId));
                            }
                            else
                            {
