...
namespace LSTY.Sdtd.WebApi.Permission
{
    ...
    public class PermissionValidator : IPermissionValidator
    {
        ...
        public PermissionValidator(IPermissionRepository permissionRepository, IMemoryCache cache)
        {
        ...
        {
            string routeStarts = string.IsNullOrEmpty(routePattern) ? "/" : "/" + routePattern + "/";
            string key = roleId + ":" + routeStarts;
            if (_cache.TryGetValue(key, out bool cacheEntry) == false)
            {
                // Key 不存在，查询数据库获取
                cacheEntry = await _permissionRepository.CheckPermissionAsync(roleId, (byte)permissionTypeRequired, routeStarts);
